...
export const FeaturedCarousel: React.FC<FeaturedCarouselProps> = ({ posts = [] }) => {
  ...
  const mainTitle = getLocalizedBlogField(mainPost, 'title', language);
  const mainExcerpt = getLocalizedBlogField(mainPost, 'excerpt', language);
  const mainCategory = getLocalizedBlogField(mainPost, 'category', language);
  ...
  return (
    <section className="w-full mb-16">
      <div className="flex flex-col items-start mb-8">
        <span className="text-[10px] font-black uppercase text-[#1e4b64] tracking-[0.28em] mb-2">{language === 'en' ? 'HIGHLIGHTS' : 'TIÊU ĐIỂM'}</span>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 leading-none">{language === 'en' ? 'Featured Articles' : 'Bài Viết Nổi Bật'}</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          >
            ...
            <div className="relative z-10 p-6 sm:p-8 flex flex-col max-w-3xl">
              ...
              <div className="flex items-center gap-1 text-[11px] font-black uppercase tracking-widest text-sky-400 group-hover:text-sky-300 transition-colors">
                <span>{language === 'en' ? 'Read article' : 'Đọc bài'}</span>
        ...
        <div className="lg:col-span-4 flex flex-col justify-start gap-6">
          {sidePosts.map((post) => {
            const title = getLocalizedBlogField(post, 'title', language);
            const category = getLocalizedBlogField(post, 'category', language);
            return (
            <div
              key={post.id}
          ...
          {sidePosts.length === 0 && (
            <div className="flex-grow flex flex-col justify-center items-center p-8 text-center rounded-[24px] bg-gradient-to-br from-zinc-50 to-zinc-100 border border-zinc-200">
              ...
              <h4 className="font-extrabold text-zinc-700 max-w-xs leading-snug mb-4">{language === 'en' ? 'Follow fresh daily insights from UR Sport' : 'Theo dõi những kiến thức mới hàng ngày tại UR Sport'}</h4>
              <button onClick={() => navigate('/shop')} className="px-6 py-2.5 rounded-full text-xs font-black uppercase tracking-widest bg-zinc-900 hover:bg-[#1e4b64] text-white transition-all shadow-sm">{language === 'en' ? 'Shop now' : 'Mua sắm ngay'}</button>
            </div>
          )}
        </div>
      </div>

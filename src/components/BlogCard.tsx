...
export const BlogCard: React.FC<BlogCardProps> = ({ post }) => {
  ...
  const title = getLocalizedBlogField(post, 'title', language, 'Bài viết');
  const excerpt = getLocalizedBlogField(post, 'excerpt', language);
      ...
      <div className="p-6 flex-grow flex flex-col">
        <div className="flex flex-wrap items-center gap-3 text-zinc-400 text-[11px] font-bold uppercase tracking-wider mb-3">
          <div className="flex items-center gap-1">
            ...
            <span>{post.date || (language === 'en' ? 'Recent' : 'Gần đây')}</span>
          </div>
